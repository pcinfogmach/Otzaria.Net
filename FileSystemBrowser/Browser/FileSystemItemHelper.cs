    ...
    public static class FileSystemItemHelper
    {
        ...
        public static async Task LoadFilesContentHeaders(string rootPath, List<FileSystemItem> items)
        {
            ...
            foreach (var item in validItems)
                loadTasks.Add(LoadFileContentHeaders(rootPath, item, loadTagsOnly: true, useFullName: true));

            await Task.WhenAll(loadTasks);
        }

        public static async Task<string> LoadFileContentHeaders(string rootPath, FileSystemItem fileSystemItem, bool loadTagsOnly, bool useFullName)
        {
                ...
                fileSystemItem.Children = await Task.Run(() =>
                {
                    ...
                    using (var reader = new StreamReader(fileSystemItem.Path))
                    {
                        ...
                        while (!reader.EndOfStream)
                        {
                            ...
                            if (!loadTagsOnly) stringBuilder.Append($"<line id=\"{index}\">{line} </line>");
                        }
